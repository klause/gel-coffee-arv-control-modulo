...
// a derived class for a "push-on, push-off" (toggle) type button and a led wired in the same pin
// initial state can be given, default is off (false).
class ToggleButtonLed : public ToggleButton
{
    public:
    
        // constructor is similar to Button, but includes the initial state for the toggle.
        ToggleButtonLed(uint8_t pin, bool initialState=false, uint32_t dbTime=25, uint8_t puEnable=true, uint8_t invert=true)
            : ToggleButton(pin, dbTime, puEnable, invert), m_toggleState(initialState) {}
        ...
        // force (set) toggle state
        bool setToggleState(bool toggleState) {
            m_lastChange = millis();
    ...
    private:
        ...
        // turn on led if toggle status is on
        void checkLed() {
            if (m_toggleState && isReleased()) {
